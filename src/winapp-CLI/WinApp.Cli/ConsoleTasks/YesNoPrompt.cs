...
public sealed class YesNoPrompt : IPrompt<(int, string)>
{
    ...
    /// <inheritdoc/>
    public (int, string) Show(IAnsiConsole console)
    {
        return ShowAsync(console, CancellationToken.None).GetAwaiter().GetResult();
    }

    /// <inheritdoc/>
    public async Task<(int, string)> ShowAsync(IAnsiConsole console, CancellationToken cancellationToken)
    {
        ...
        var compare = comparer.Compare(Yes.ToString(), result.ToString());
        return (compare, result.ToString());
    }
}
